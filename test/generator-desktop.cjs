...
(async () => {
  ...
  try {
    ...
    assert.match(await prompt.inputValue(), /@Purple reference/); assert.equal(await page.locator('#mention-menu').isVisible(), false);
    await prompt.press('Enter'); await page.waitForFunction(() => document.querySelectorAll('.reference-chip').length === 0);
    ...
    await reopened.waitForFunction(() => document.querySelectorAll('.generation-tile.ready').length === 5);
